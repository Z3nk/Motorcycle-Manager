...
@Database(
    entities = [BikeEntity::class, ConsumableEntity::class, CheckEntity::class, HistoryEntity::class, InvoiceEntity::class],
    version = 6,
    exportSchema = true
)
abstract class MotorCycleDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        """.trimIndent()
                ...
                db.execSQL(
                    """
            CREATE INDEX IF NOT EXISTS index_invoices_bikeId 
            ON invoices (bikeId)
        """.trimIndent()
                )
            }
